...
def train_output_range_dtype(s2, dem, s1, feats):
    """
    ...
    assert np.logical_and(
        feats[..., 1:65].min() >= -32.768, feats[..., 1:65].max() <= 32.768
    ), print(feats[..., 1:65].min(), feats[..., 1:65].max())
    assert np.logical_and(feats[..., 0].min() >= 0, feats[..., 0].max() <= 1), print(
        feats[..., 0].min(), feats[..., 0].max()
    )
...
def input_dtype_and_dimensions(tile_idx, country):
    ...
    assert (
        s2_20.shape[3] == 6 or s2_20.shape[3] == 7
    )  # 7 indices if data mask is included
...
def feats_range(tile_idx, country):
    ...
    assert (
        np.logical_and(feats[0, ...].min() >= 0, feats[0, ...].max() <= 100)
        or feats[0, ...].max() == 255
    )
    assert np.logical_and(
        feats.min() >= np.iinfo(np.int16).min, feats.max() <= np.iinfo(np.int16).max
    )
...
def output_dtype_and_dimensions(s1, s2, dem):
    ...
    assert s1.shape[0:2] == s2.shape[0:2] == dem.shape, print(
        f"S1: {s1.shape} \n" f"S2: {s2.shape} \n" f"DEM: {dem.shape}"
    )
...
def tmlfeats_dtype_and_dimensions(dem, feats, feature_select):
    ...
    assert feats.dtype == np.float32
    assert feats.shape[0:2] == dem.shape, print(
        f"WARNING. Feats: {feats.shape} DEM: {dem.shape}"
    )
...
def model_outputs(arr, type):
    ...
    if type == "classifier":
        assert np.logical_or.reduce(
            (arr == 0, arr == 1, arr == 2, arr == 3)
        ).all() or np.any(arr == 255)
        assert arr.all() != 1

    elif type == "regressor":
        assert np.logical_and(arr >= 0, arr <= 100).all() or np.any(arr == 255), print(
            np.unique(arr)
        )
