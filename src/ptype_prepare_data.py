...
def load_ttc(idx, ard):
    ...
    '''
    if ard:
        directory = '../data/train-features-ard/'
    else:
        directory = '../data/train-features-ckpt-2023-02-09/'
...
def load_label(idx, ttc, classes, directory = '../data/train-labels/'):
    '''
...
def load_txt(idx, ard, directory = '../data/train-texture/'):
    '''
    ...
    '''
    if ard:
        if os.path.exists(f'{directory}{idx}_ard.npy'):
            output = np.load(f'{directory}{idx}_ard.npy')
        else:
...
# def make_sample_ard(sample_shape, ard, txt, ttc, feature_select):
...
def create_xy(v_train_data, classes, feature_select, sample_shape=(14, 14), verbose=False):
    ...
    ard, raw = gather_plot_ids(v_train_data)
    ...
    for num, plot in enumerate(tqdm(ard)):
        ard = load_ard(plot)
        ttc = load_ttc(plot, ard=True)
        txt = load_txt(plot, ard=True) 
    ...
    for num, plot in enumerate(tqdm(raw)):
        ...
        s1 = load_s1(plot)
        s2 = load_s2(plot)
        ttc = load_ttc(plot, ard=False)
        txt = load_txt(plot, ard=False)
