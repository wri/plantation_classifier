...
def load_slope(idx, local_data_path):
    """
    ...
    """
    directory = local_data_path + 'train-slope/'
...
def load_s1(idx, local_data_path):
    """
    ...
    directory = local_data_path + 'train-s1/'
...
def load_s2(idx, local_data_path):
    ...
    Convert to float32.
    """
    directory = local_data_path + 'train-s2/'
...
def load_ttc(idx, local_data_path):
    '''
    ...
    '''
    directory = local_data_path + 'train-features-ckpt-2023-02-09/'
...
def load_txt(idx, local_data_path):
    ...
    if os.path.exists(f'{local_data_path}/train-texture/{idx}.npy'):
        output = np.load(f'{local_data_path}/train-texture/{idx}.npy')
...
def create_xy(v_train_data, classes, drop_feats, config_path, feature_select=[], verbose=False):
    ...
    local_data_path = config['data_load']['local_prefix']
    for num, plot in enumerate(tqdm(plot_ids)):

        if drop_feats:
            slope = load_slope(plot, local_data_path)
            s1 = load_s1(plot, local_data_path)
            s2 = load_s2(plot, local_data_path)
            ...
            s1 = load_s1(plot, local_data_path)
            s2 = load_s2(plot, local_data_path)
            ttc = load_ttc(plot, local_data_path)
            txt = load_txt(plot, local_data_path)
