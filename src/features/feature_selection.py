...
def least_imp_feature(model, X_test, logger):
    ...
    logger.debug(f"columns: {importance_df.features}")
...
def backward_selection(
    ...
    for num_features in range(total_features - 1, 1, -1):
        ...
        logger.info(
            f"Removed feature {dropped_feature}, {tmp_X_train.shape[1]} features remaining"
        )
        ...
        if (num_features < max_features) and (
            metric < last_metric
        ):  # consider that this could exceed max featuers without warning
...
def backward_selection_v2(
    ...
    for i in range(total_features - 1, -1, -1):
        ...
        logger.info(
            f"Removed feature {dropped_feature}, {X_train.shape[1]} features remaining"
        )
