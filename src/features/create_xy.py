...
def use_select_feats(X_train: np.array, X_test: np.array, feature_select: list):
    ...
    X_train_fs = np.zeros(
        shape=(X_train.shape[0], X_train.shape[1], X_train.shape[2], n_feats)
    )

    X_test_fs = np.zeros(
        shape=(X_test.shape[0], X_test.shape[1], X_test.shape[2], n_feats)
    )
...
def prepare_model_inputs(X, y, params_path, logger, feature_select):
    ...
    X_train_scaled, X_test_scaled = scale_arr(
        X_train, X_test, filepath_mins, filepath_maxs
    )
    ...
    logger.debug(f"Reshaped, unscaled X_train: {X_train.shape} X_test: {X_test.shape}")
    logger.debug(
        f"Reshaped, scaled X_train: {X_train_scaled.shape} X_test: {X_test_scaled.shape}"
    )
